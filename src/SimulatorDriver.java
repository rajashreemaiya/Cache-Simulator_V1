...
public class SimulatorDriver {
	...
	public static void main(String[] args) {
		...
		int sizeOfClientCache = Integer.parseInt(prop
				.getProperty("clientBlockSize"));

		SimulatorConstants.SEARCH = Integer
				.parseInt(prop.getProperty("search"));
		...
		SimulatorConstants.TODISK = Integer
				.parseInt(prop.getProperty("toDisk"));
		...
		for (int i = 0; i < SimulatorContentManager.allClients.length; i++) {
			try {
				...
				thread.join();
				totalTicks.add(SimulatorContentManager.allClients[i].mytickCount
								.getTickCount());
				inLocalCache.add(SimulatorContentManager.allClients[i].LocalcacheHits);
				inNeighborCache.add(SimulatorContentManager.allClients[i].NeighborcacheHits);
				cacheMiss.add(SimulatorContentManager.allClients[i].cacheMiss);
		...
		System.out.println("For " + numOfClients
		 ...
		 FileWriter writer = new FileWriter("Output.txt",true);
			 writer.append("\n"+numOfClients+", " + sum + ", "+ sum0 + ", "+sum1 + ", "+ sum2);
			 writer.flush();
			 writer.close();
		 } catch (IOException e) {
		 e.printStackTrace();
